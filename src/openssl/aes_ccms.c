             ...
             uint8_t *key)
{
    ...
    if (aes_cbc_mac_enc(a, len_a, m, *len_m, saddr, asn, key, mac, CBC_MAC_SIZE) == 0)
    {
        if (aes_ctr_enc(m, *len_m, saddr, asn, key, mac, CBC_MAC_SIZE) == 0)
        {
                 ...
                 uint8_t *key)
{
    ...
    *len_m -= CBC_MAC_SIZE;
    ...
    if (aes_ctr_enc(m, *len_m, saddr, asn, key, mac, CBC_MAC_SIZE) == 0)
    {
        if (aes_cbc_mac_enc(a, len_a, m, *len_m, saddr, asn, key, orig_mac, CBC_MAC_SIZE) == 0)
        {
