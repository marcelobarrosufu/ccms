...
int aes_ctr_enc_fw(uint8_t *buffer, uint8_t len, AES_KEY *key_enc, uint8_t iv[16])
{
    uint8_t n, k, nb, *pbuf;
    uint8_t eiv[16];
    ...
    uint8_t aes_ctr_support)
{
    ...
    if (!((aes_ctr_support == AES_CTR_HW) || (aes_ctr_support == AES_CTR_FW)))
        return -4;
    ...
    if (aes_ctr_support == AES_CTR_HW)
        ...
        aes_ctr_enc_fw(buffer, len, &key_enc, iv);
